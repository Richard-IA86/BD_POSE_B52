...
_COSTOS_REQUERIDAS = [
    "FECHA", "OBRA_PRONTO", "IMPORTE",
    "anio_dato", "mes_dato", "periodo_codigo",
]

_COMPROBANTES_REQUERIDAS = [
    "FECHA_COMPROBANTE", "NRO_COMPROBANTE", "IMPORTE",
    "anio_dato",
]
...
def validar_schema_costos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if descartados:
        logging.warning("Costos: %d filas descartadas (FECHA/IMPORTE/OBRA_PRONTO nulo)", descartados)
...
def validar_schema_comprobantes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_antes = len(df)
    df_valido = df.dropna(subset=["FECHA_COMPROBANTE", "NRO_COMPROBANTE", "IMPORTE"]).copy()
    descartados = n_antes - len(df_valido)
    if descartados:
        logging.warning(
