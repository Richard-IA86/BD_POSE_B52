...
def procesar_obras_gerencias(conn):
    ...
    df['OBRA_PRONTO'] = df['OBRA_PRONTO'].apply(
        lambda x: str(x).strip().zfill(8) if str(x).strip().isdigit() else str(x).strip() if x else None
    )
    ...
    for _, row in df_obras.iterrows():
        ...
        # Check si ya existe
        cursor.execute("SELECT 1 FROM CATALOGO.obras WHERE obra_pronto = ?", (obra_pronto,))
        if cursor.fetchone():
        ...
        cursor.execute(
            """INSERT INTO CATALOGO.obras 
               ...
               VALUES (?, ?, ?, ?, ?, 1)""",
            obra_pronto,
            str(row.get('DESCRIPCION_OBRA', ''))[:600] if pd.notna(row.get('DESCRIPCION_OBRA')) else None,
...
def procesar_dimensiones_dinamicas(conn):
    ...
    df = pd.read_excel(FILE_COSTOS, usecols=['FUENTE', 'TIPO_COMPROBANTE', 'RUBRO_CONTABLE', 'CODIGO_CUENTA', 'CUENTA_CONTABLE'],
    ...
    tc_data = [(str(row['TIPO_COMPROBANTE']).upper().strip(),) for _, row in df_tc.iterrows()]
    upsert_catalogo(conn, 'CATALOGO.tipos_comprobantes', ['tipo_comprobante'], tc_data, 'tipo_comprobante')
    ...
    cc_data = [
        (str(row['RUBRO_CONTABLE']).upper().strip()[:150] if pd.notna(row.get('RUBRO_CONTABLE')) else None,
         ...
         str(row['CUENTA_CONTABLE']).upper().strip()[:400] if pd.notna(row.get('CUENTA_CONTABLE')) else None)
