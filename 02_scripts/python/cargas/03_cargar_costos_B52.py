...
COLUMNAS_ESPERADAS = {
    "OBRA_PRONTO", "FECHA", "IMPORTE", "TC", "IMPORTE_USD",
    "GERENCIA", "PROVEEDOR", "NRO_COMPROBANTE", "TIPO_COMPROBANTE",
    "OBSERVACION", "RUBRO_CONTABLE", "CUENTA_CONTABLE",
    "CODIGO_CUENTA", "COMPENSABLE", "FUENTE", "DESCRIPCION_OBRA", "DETALLE",
}
...
LOG_FILE = LOG_DIR / f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_03_costos.log"
logging.basicConfig(
    level=logging.INFO,
...
def leer_y_normalizar() -> pd.DataFrame:
    logging.info("Leyendo: %s  (%.1f MB)",
                 ARCHIVO_COSTOS, ARCHIVO_COSTOS.stat().st_size / 1_048_576)

    # Leer importes como string para evitar pérdida de precisión (igual que A2)
    dtype_str = {"IMPORTE": str, "TC": str, "IMPORTE_USD": str}
    try:
        df = pd.read_excel(ARCHIVO_COSTOS, sheet_name=HOJA_COSTOS,
                           engine="openpyxl", dtype=dtype_str)
        logging.info("Hoja '%s' cargada.", HOJA_COSTOS)
    except Exception:
        logging.warning("Hoja '%s' no encontrada — usando índice 0.", HOJA_COSTOS)
        df = pd.read_excel(ARCHIVO_COSTOS, sheet_name=0,
                           engine="openpyxl", dtype=dtype_str)
    ...
    df.columns = (
        df.columns.str.upper().str.strip()
    ...
    # Convertir tipos
    df["FECHA"]       = df["FECHA"].apply(_conv_fecha) if "FECHA" in df.columns else None
    df["IMPORTE"]     = df["IMPORTE"].apply(_conv_num)      if "IMPORTE"     in df.columns else None
    df["TC"]          = df["TC"].apply(_conv_num)           if "TC"          in df.columns else None
    df["IMPORTE_USD"] = df["IMPORTE_USD"].apply(_conv_num)  if "IMPORTE_USD" in df.columns else None
...
def validar_fila(row, idx, obras_validas: set):
    ...
    elif obras_validas and str(row["OBRA_PRONTO"]).strip() not in obras_validas:
        errores.append(f"OBRA_PRONTO no existe en catálogo: [{row['OBRA_PRONTO']}]")
...
def _insertar_rows(conn, id_log: int, df_batch: pd.DataFrame,
                   fuente_map: dict, obra_map: dict) -> None:
    """
    ...
    for idx, row in df_batch.iterrows():
        ...
        if not id_obra and obra_pronto_str:
            logging.warning("Fila %d: obra_pronto '%s' no existe en catálogo (skipped)",
                          idx, obra_pronto_str)
...
def sincronizar_fuentes(conn, df: pd.DataFrame) -> dict:
    ...
    } if "FUENTE" in df.columns else set()

    cursor = conn.cursor()
    cursor.execute("SELECT codigo_fuente, id_fuente FROM CATALOGO.fuentes WHERE activo=1")
    ...
    if nuevas:
        conn.commit()
        logging.info("Fuentes sincronizadas: %d nuevas, %d totales.", len(nuevas), len(fuente_map))
    else:
        logging.info("Fuentes en Excel: %d — todas ya registradas (%d en catálogo).",
                     len(fuentes_excel), len(fuente_map))
    return fuente_map
...
def borrar_periodo(conn, anio: int, mes: int) -> int:
    cursor = conn.cursor()
    cursor.execute(
        "SELECT COUNT(*) FROM PRODUCCION.costos WHERE anio_dato=? AND mes_dato=?", anio, mes
    )
    n = cursor.fetchone()[0]
    if n > 0:
        cursor.execute(
            "DELETE FROM PRODUCCION.costos WHERE anio_dato=? AND mes_dato=?", anio, mes
        )
        conn.commit()
    ...
    for idx, motivo, row in rechazos:
        cursor.execute(
            """INSERT INTO AUDITORIA.rechazos
               ...
               VALUES (?,?,?,?)""",
            id_log, int(idx) + 2, motivo,
            json.dumps(row.to_dict(), default=str, ensure_ascii=False),
        )
    conn.commit()
    logging.info("  %d rechazos registrados en AUDITORIA.rechazos", len(rechazos))


def cargar_periodo(conn, df: pd.DataFrame, obras_validas: set,
                   fuente_map: dict, obra_map: dict, anio: int, mes: int,
    ...
    if not force:
        ya, id_prev = verificar_procesado_periodo(conn, "PRODUCCION.costos", periodo_codigo)
        if ya:
            logging.info("  ⚠ Ya procesado (id_periodo=%s) — use --force.", id_prev)
            return

    medidor = MedidorRendimiento(f"costos_{periodo_codigo}")
    medidor.iniciar()
    id_log, id_periodo = registrar_inicio_periodo(
        conn, "PRODUCCION.costos", "MENSUAL", anio, mes, periodo_codigo, USUARIO
    )
    try:
        ...
        logging.info("  Válidas: %d | Rechazadas: %d", len(validas), len(rechazos))

        if not validas:
            _registrar_rechazos(conn, id_log, rechazos)
            registrar_fin_periodo(conn, id_log, id_periodo, 0, 0,
                                  0.0, "VACIO", "Sin filas válidas")
            return
        ...
        for i in range(0, len(df_validas), BATCH_SIZE):
            ...
            logging.info("  Inserción: %d/%d (%.0f%%)",
                         insertados, len(df_validas), 100 * insertados / len(df_validas))
        ...
        if filas_en_bd != insertados:
            logging.warning(
                ...
                periodo_codigo, insertados, filas_en_bd,
            )
            estado_final = "ADVERTENCIA"
        else:
            logging.info("  ✔ Verificación OK: %d filas en BD == %d insertadas",
                         filas_en_bd, insertados)
        ...
        registrar_fin_periodo(
            ...
            medidor.duracion_total, estado_final,
            f"Val:{len(validas)} Rech:{len(rechazos)} BD:{filas_en_bd} Vel:{vel:.0f}reg/s",
        )
        logging.info("  ✅ %s — %d insertados, %d rechazados, %.1fs",
                     periodo_codigo, insertados, len(rechazos), medidor.duracion_total)
        medidor.imprimir_resumen()

    except Exception as exc:
        medidor.finalizar()
        registrar_fin_periodo(conn, id_log, id_periodo, 0, 0,
                              medidor.duracion_total, "ERROR", str(exc)[:500])
...
def recuperar_rechazos_obra(conn, obra_pronto: str, df: pd.DataFrame,
                            obras_validas: set, fuente_map: dict, obra_map: dict) -> None:
    ...
    if not periodos:
        logging.warning("No se encontraron rechazos para obra '%s'. "
                        "¿Ya fue recuperada o el código es incorrecto?", obra_pronto)
        return
    ...
    logging.info("Obra '%s': %d período(s) con rechazos → forzando recarga.",
                 obra_pronto, len(periodos))
    for anio, mes in periodos:
        cargar_periodo(conn, df, obras_validas, fuente_map, obra_map, int(anio), int(mes), force=True)
...
def main():
    parser = argparse.ArgumentParser(description="Carga incremental MENSUAL de costos B52")
    parser.add_argument("--periodos", type=str, help="YYYYMM,YYYYMM,...")
    parser.add_argument("--full",  action="store_true", help="Todos los períodos del archivo")
    parser.add_argument("--force", action="store_true", help="Re-cargar aunque ya esté procesado")
    parser.add_argument("--recuperar-obra", type=str, metavar="OBRA_PRONTO",
    ...
    try:
        ...
        logging.info("Catálogo: %d obras activas cargadas.", len(obras_validas))
        ...
        if args.recuperar_obra:
            ...
            recuperar_rechazos_obra(conn, obra, df, obras_validas, fuente_map, obra_map)
        elif args.full:
            unicos = (
                df[["anio_dato", "mes_dato"]].dropna()
                .drop_duplicates()
                .sort_values(["anio_dato", "mes_dato"])
            )
            logging.info("Modo FULL: %d períodos a procesar", len(unicos))
            for _, row in unicos.iterrows():
                cargar_periodo(conn, df, obras_validas, fuente_map, obra_map,
                               int(row["anio_dato"]), int(row["mes_dato"]), args.force)
        elif args.periodos:
            for p in args.periodos.split(","):
                ...
                if len(p) != 6 or not p.isdigit():
                    ...
                    continue
                cargar_periodo(conn, df, obras_validas, fuente_map, obra_map,
                               int(p[:4]), int(p[4:]), args.force)
        else:
            logging.error("Especificar --periodos YYYYMM,... o --full o --recuperar-obra OBRA")
            sys.exit(1)
    finally:
        conn.close()
